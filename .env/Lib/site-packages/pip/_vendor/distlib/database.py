...
class _Cache(object):
    ...
    def add(self, dist):
        ...
        """
        if dist.DIR_PATH not in self.path:
            self.path[dist.DIR_PATH] = dist
...
class DistributionPath(object):
    ...
    def _yield_distributions(self):
        ...
        for path in self.path:
            ...
            for entry in rset:
                r = finder.find(entry)
                if not r or r.DIR_PATH in seen:
                    continue
                try:
                    if self._include_dist and entry.endswith(DISTINFO_EXT):
                        ...
                        with contextlib.closing(pydist.as_stream()) as stream:
                            metadata = Metadata(fileobj=stream,
                                                scheme='legacy')
                        logger.debug('Found %s', r.DIR_PATH)
                        seen.add(r.DIR_PATH)
                        yield new_dist_class(r.DIR_PATH,
                                             metadata=metadata,
                                             env=self)
                        ...
                        logger.debug('Found %s', r.DIR_PATH)
                        seen.add(r.DIR_PATH)
                        yield old_dist_class(r.DIR_PATH, self)
                except Exception as e:
                    msg = 'Unable to read distribution at %s, perhaps due to bad metadata: %s'
                    logger.warning(msg, r.DIR_PATH, e)
                    import warnings
                    warnings.warn(msg % (r.DIR_PATH, e), stacklevel=2)
...
class InstalledDistribution(BaseInstalledDistribution):
    ...
    def __init__(self, path, metadata=None, env=None):
        ...
        if env and env._cache_enabled and path in env._cache.DIR_PATH:
            metadata = env._cache.DIR_PATH[path].metadata
...
class EggInfoDistribution(BaseInstalledDistribution):
    ...
    def __init__(self, path, env=None):
        ...
        self.path = path
        self.dist_path = env
        if env and env._cache_enabled and path in env._cache_egg.DIR_PATH:
            metadata = env._cache_egg.DIR_PATH[path].metadata
